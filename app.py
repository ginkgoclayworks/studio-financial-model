...
def _num(label, key, default=None, min_value=None, max_value=None, step=None, help=None, fmt=None):
    ...
    return st.number_input(label, key=key, value=st.session_state[key], **{k: v for k, v in kw.items() if v is not None}, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
...
def render_advanced_controls(defaults: dict) -> dict:
    ...
    with st.sidebar.expander('Capacity & Hours', expanded=True):
        ...
        adv['SESSIONS_PER_WEEK'] = _num('Sessions per week', 'adv_SESSIONS_PER_WEEK', defaults.get('SESSIONS_PER_WEEK', help='Member behavior assumptions by archetype. Adjust only if you have measured data.'), 1, 70, 1, help='Member behavior assumptions by archetype. Adjust only if you have measured data.')
        adv['SESSION_HOURS'] = _num('Hours per session', 'adv_SESSION_HOURS', defaults.get('SESSION_HOURS', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0.5, 12.0, 0.5, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['OPEN_HOURS_PER_WEEK'] = _num('Open hours per week', 'adv_OPEN_HOURS_PER_WEEK', defaults.get('OPEN_HOURS_PER_WEEK', help='Operational capacity assumptions used to compute membership limits.'), 1, 168, 1, help='Operational capacity assumptions used to compute membership limits.')
        adv['CAPACITY_DAMPING_BETA'] = _num('Capacity damping β (higher = softer cap)', 'adv_CAPACITY_DAMPING_BETA', defaults.get('CAPACITY_DAMPING_BETA', help='Operational capacity assumptions used to compute membership limits.'), 0.0, 10.0, 0.1, help='Operational capacity assumptions used to compute membership limits.')
    with st.sidebar.expander('Top-of-Funnel & Referrals', expanded=True):
        adv['BASELINE_RATE_HOME'] = _num('Baseline join rate (Home potters)', 'adv_BASELINE_RATE_HOME', defaults.get('BASELINE_RATE_HOME', help='Fractional rate (0–1). For example, 6% = 0.06.'), 0.0, 1.0, 0.001, help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['BASELINE_RATE_COMMUNITY'] = _num('Baseline join rate (Community studio users)', 'adv_BASELINE_RATE_COMMUNITY', defaults.get('BASELINE_RATE_COMMUNITY', help='Fractional rate (0–1). For example, 6% = 0.06.'), 0.0, 1.0, 0.001, help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['BASELINE_RATE_NO_ACCESS'] = _num('Baseline join rate (No access)', 'adv_BASELINE_RATE_NO_ACCESS', defaults.get('BASELINE_RATE_NO_ACCESS', help='Fractional rate (0–1). For example, 6% = 0.06.'), 0.0, 1.0, 0.001, help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['HOME_INFLOW'] = _num('Monthly inflow (Home)', 'adv_HOME_INFLOW', defaults.get('HOME_INFLOW', help='Market dynamics: higher values speed up growth; churn reduces active members.'), 0, 100000, 10, help='Market dynamics: higher values speed up growth; churn reduces active members.')
        adv['COMMUNITY_INFLOW'] = _num('Monthly inflow (Community)', 'adv_COMMUNITY_INFLOW', defaults.get('COMMUNITY_INFLOW', help='Market dynamics: higher values speed up growth; churn reduces active members.'), 0, 100000, 10, help='Market dynamics: higher values speed up growth; churn reduces active members.')
        adv['NO_ACCESS_INFLOW'] = _num('Monthly inflow (No access)', 'adv_NO_ACCESS_INFLOW', defaults.get('NO_ACCESS_INFLOW', help='Market dynamics: higher values speed up growth; churn reduces active members.'), 0, 100000, 10, help='Market dynamics: higher values speed up growth; churn reduces active members.')
        adv['REFERRAL_RATE_PER_MEMBER'] = _num('Referrals per member per month', 'adv_REFERRAL_RATE_PER_MEMBER', defaults.get('REFERRAL_RATE_PER_MEMBER', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0.0, 2.0, 0.01, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['REFERRAL_CONV'] = _num('Referral conversion rate (0–1)', 'adv_REFERRAL_CONV', defaults.get('REFERRAL_CONV', help='Fractional rate (0–1). For example, 6% = 0.06.'), 0.0, 1.0, 0.01, help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['MARKETING_COST_BASE'] = _num('Marketing base cost ($/mo)', 'adv_MARKETING_COST_BASE', defaults.get('MARKETING_COST_BASE', help='Enter the monthly dollar amount. Use 0 if not applicable.'), 0, 50000, 50, help='Enter the monthly dollar amount. Use 0 if not applicable.')
        adv['MARKETING_RAMP_MONTHS'] = _num('Marketing ramp months', 'adv_MARKETING_RAMP_MONTHS', defaults.get('MARKETING_RAMP_MONTHS', help='Time length for this setting. Choose years unless otherwise noted.'), 0, 36, 1, help='Time length for this setting. Choose years unless otherwise noted.')
        adv['MARKETING_RAMP_MULTIPLIER'] = _num('Marketing ramp multiplier', 'adv_MARKETING_RAMP_MULTIPLIER', defaults.get('MARKETING_RAMP_MULTIPLIER', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0.0, 10.0, 0.1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
    with st.sidebar.expander('Firing & Utilities', expanded=False):
        adv['BASE_FIRINGS_PER_MONTH'] = _num('Base firings / month', 'adv_BASE_FIRINGS_PER_MONTH', defaults.get('BASE_FIRINGS_PER_MONTH', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 60, 1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['MIN_FIRINGS_PER_MONTH'] = _num('Min firings / month', 'adv_MIN_FIRINGS_PER_MONTH', defaults.get('MIN_FIRINGS_PER_MONTH', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 60, 1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['MAX_FIRINGS_PER_MONTH'] = _num('Max firings / month', 'adv_MAX_FIRINGS_PER_MONTH', defaults.get('MAX_FIRINGS_PER_MONTH', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 120, 1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['DYNAMIC_FIRINGS'] = _num('Dynamic firings (0=off,1=on)', 'adv_DYNAMIC_FIRINGS', defaults.get('DYNAMIC_FIRINGS', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 1, 1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['REFERENCE_MEMBERS_FOR_BASE_FIRINGS'] = _num('Reference members for base firings', 'adv_REFERENCE_MEMBERS_FOR_BASE_FIRINGS', defaults.get('REFERENCE_MEMBERS_FOR_BASE_FIRINGS', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 1, 500, 1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['KWH_PER_FIRING_KMT1027'] = _num('kWh per firing (KMT 1027)', 'adv_KWH_PER_FIRING_KMT1027', defaults.get('KWH_PER_FIRING_KMT1027', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0.0, 5000.0, 1.0, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['KWH_PER_FIRING_KMT1427'] = _num('kWh per firing (KMT 1427)', 'adv_KWH_PER_FIRING_KMT1427', defaults.get('KWH_PER_FIRING_KMT1427', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0.0, 5000.0, 1.0, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['WATER_COST_PER_GALLON'] = _num('Water cost ($/gal)', 'adv_WATER_COST_PER_GALLON', defaults.get('WATER_COST_PER_GALLON', help='Enter the monthly dollar amount. Use 0 if not applicable.'), 0.0, 10.0, 0.01, help='Enter the monthly dollar amount. Use 0 if not applicable.')
    with st.sidebar.expander('Classes & Events', expanded=False):
        adv['CLASS_INSTR_RATE_PER_HR'] = _num('Instructor rate ($/hr)', 'adv_CLASS_INSTR_RATE_PER_HR', defaults.get('CLASS_INSTR_RATE_PER_HR', help='Fractional rate (0–1). For example, 6% = 0.06.'), 0, 500, 1, help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['CLASS_HOURS_PER_COHORT'] = _num('Hours per cohort', 'adv_CLASS_HOURS_PER_COHORT', defaults.get('CLASS_HOURS_PER_COHORT', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 200, 1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['CLASS_FILL_MEAN'] = _num('Class fill (avg seats)', 'adv_CLASS_FILL_MEAN', defaults.get('CLASS_FILL_MEAN', help='Controls revenue/throughput for this activity. Adjust only if you run this offering.'), 0, 50, 1, help='Controls revenue/throughput for this activity. Adjust only if you run this offering.')
        adv['CLASS_COST_PER_STUDENT'] = _num('Variable cost per student ($)', 'adv_CLASS_COST_PER_STUDENT', defaults.get('CLASS_COST_PER_STUDENT', help='Enter the monthly dollar amount. Use 0 if not applicable.'), 0, 2000, 1, help='Enter the monthly dollar amount. Use 0 if not applicable.')
        adv['EVENT_CONV_RATE'] = _num('Event conversion to member (0–1)', 'adv_EVENT_CONV_RATE', defaults.get('EVENT_CONV_RATE', help='Controls revenue/throughput for this activity. Adjust only if you run this offering.'), 0.0, 1.0, 0.01, help='Controls revenue/throughput for this activity. Adjust only if you run this offering.')
        adv['EVENT_CONV_LAG_MO'] = _num('Event conversion lag (months)', 'adv_EVENT_CONV_LAG_MO', defaults.get('EVENT_CONV_LAG_MO', help='Time length for this setting. Choose years unless otherwise noted.'), 0, 12, 1, help='Time length for this setting. Choose years unless otherwise noted.')
        adv['WORKSHOP_CONV_RATE'] = _num('Workshop conversion (0–1)', 'adv_WORKSHOP_CONV_RATE', defaults.get('WORKSHOP_CONV_RATE', help='Controls revenue/throughput for this activity. Adjust only if you run this offering.'), 0.0, 1.0, 0.01, help='Controls revenue/throughput for this activity. Adjust only if you run this offering.')
        adv['WORKSHOP_CONV_LAG_MO'] = _num('Workshop conversion lag (months)', 'adv_WORKSHOP_CONV_LAG_MO', defaults.get('WORKSHOP_CONV_LAG_MO', help='Time length for this setting. Choose years unless otherwise noted.'), 0, 12, 1, help='Time length for this setting. Choose years unless otherwise noted.')
    with st.sidebar.expander('Financing', expanded=False):
        adv['LOAN_ANNUAL_RATE'] = _num('Loan annual rate (APR)', 'adv_LOAN_ANNUAL_RATE', defaults.get('LOAN_ANNUAL_RATE', help='Fractional rate (0–1). For example, 6% = 0.06.'), 0.0, 0.5, 0.005, fmt='%.3f', help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['LOAN_TERM_YEARS'] = _num('Loan term (years)', 'adv_LOAN_TERM_YEARS', defaults.get('LOAN_TERM_YEARS', help='Time length for this setting. Choose years unless otherwise noted.'), 0, 20, 1, help='Time length for this setting. Choose years unless otherwise noted.')
    with st.sidebar.expander('Overheads & Taxes (advanced)', expanded=False):
        adv['INSURANCE_COST'] = _num('Insurance ($/mo)', 'adv_INSURANCE_COST', defaults.get('INSURANCE_COST', help='Enter the monthly dollar amount. Use 0 if not applicable.'), 0, 20000, 10, help='Enter the monthly dollar amount. Use 0 if not applicable.')
        adv['GLAZE_COST_PER_MONTH'] = _num('Glaze cost ($/mo)', 'adv_GLAZE_COST_PER_MONTH', defaults.get('GLAZE_COST_PER_MONTH', help='Enter the monthly dollar amount. Use 0 if not applicable.'), 0, 10000, 10, help='Enter the monthly dollar amount. Use 0 if not applicable.')
        adv['HEATING_COST_SUMMER'] = _num('Heating ($/mo) summer', 'adv_HEATING_COST_SUMMER', defaults.get('HEATING_COST_SUMMER', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 20000, 10, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['HEATING_COST_WINTER'] = _num('Heating ($/mo) winter', 'adv_HEATING_COST_WINTER', defaults.get('HEATING_COST_WINTER', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 20000, 10, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
        adv['MA_SALES_TAX_RATE'] = _num('MA sales tax rate (0–1)', 'adv_MA_SALES_TAX_RATE', defaults.get('MA_SALES_TAX_RATE', help='Fractional rate (0–1). For example, 6% = 0.06.'), 0.0, 0.15, 0.001, fmt='%.3f', help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['SALES_TAX_REMIT_FREQUENCY_MONTHS'] = _num('Sales tax remit frequency (months)', 'adv_SALES_TAX_REMIT_FREQUENCY_MONTHS', defaults.get('SALES_TAX_REMIT_FREQUENCY_MONTHS', help='Fractional rate (0–1). For example, 6% = 0.06.'), 1, 12, 1, help='Fractional rate (0–1). For example, 6% = 0.06.')
        adv['ESTIMATED_TAX_REMIT_FREQUENCY_MONTHS'] = _num('Estimated tax remit frequency (months)', 'adv_ESTIMATED_TAX_REMIT_FREQUENCY_MONTHS', defaults.get('ESTIMATED_TAX_REMIT_FREQUENCY_MONTHS', help='Fractional rate (0–1). For example, 6% = 0.06.'), 1, 12, 1, help='Fractional rate (0–1). For example, 6% = 0.06.')
    with st.sidebar.expander('Seasonality (power user)', expanded=False):
        adv['NORMALIZE_SEASONALITY'] = _num('Normalize seasonality (0=off,1=on)', 'adv_NORMALIZE_SEASONALITY', defaults.get('NORMALIZE_SEASONALITY', help='Type an exact numeric value. Use arrow keys for fine adjustments.'), 0, 1, 1, help='Type an exact numeric value. Use arrow keys for fine adjustments.')
